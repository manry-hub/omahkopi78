...
export default function AddReservationTableForm() {
    ...
    const handleUploadImage = async (e) => {
        ...
        try {
            ...
            fd.append("file", file);
            fd.append("folder", "reservation_tables"); // 🔥 ini penting
    ...
    const handleSubmit = async (e) => {
        ...
        if (!name || !minPay || !minPeople || !capacity) {
        ...
        if (res.ok) {
            alert("Meja reservasi berhasil ditambahkan!");
            window.location.href = "/admin/meja-reservasi";
        } else {
            const data = await res.json().catch(() => ({}));
            alert("Gagal tambah meja reservasi: " + (data.message || "Unknown error"));
        }
    };
