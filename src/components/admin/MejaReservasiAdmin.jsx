export default function MejaReservasiAdmin({ reservationTables = [] }) {
    const handleDelete = async (id) => {
        const ok = confirm("Yakin mau hapus kategori ini?");
        if (!ok) return;

        const res = await fetch(`/api/reservation-tables/${id}`, { method: "DELETE" });
    ...
    return (
        <div className="dashboard-main-body">
            <div className="flex flex-wrap items-center justify-between gap-2 mb-6">
                <h6 className="font-semibold mb-0 dark:text-white">Kelola Meja Reservasi</h6>
            </div>
                                ...
                                <table className="table bordered-table mb-0">
                                    ...
                                    <tbody>
                                        {reservationTables?.length > 0 ? (
                                            reservationTables.map((item, index) => (
                                                <tr key={item.id}>
                                                    <td>{index + 1}</td>
                                                    ...
                                                    <td>
                                                        <div className="flex items-center gap-2">
                                                            <a
                                                                href={`/admin/meja-reservasi/${item.id}`}
